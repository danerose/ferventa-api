...
export class MaintenanceService {
  ...
  async directReception(
    ...
    try {
      ...
      console.error(
        'Error auto-creating completed appointment during direct reception:',
        err,
      );
    }
    ...
    const maintenance = new this.maintenanceModel({
      ...
      appointment: appointment ? appointment._id.toString() : null,
  ...
  async create(
    ...
    await this.customersService.findById(
      createMaintenanceDto.customerId,
      branchId,
    );
    ...
    if (
      (vehicle.customer as any)._id.toString() !==
      createMaintenanceDto.customerId
    ) {
      const i18n = I18nContext.current();
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.vehicleNotBelongToCustomer')
          : 'El vehículo no pertenece al cliente especificado',
      );
    }
    ...
    if (createMaintenanceDto.appointmentId) {
      const appt = await this.appointmentModel
        .findOne({ _id: createMaintenanceDto.appointmentId, branch: branchId })
        .exec();
      if (!appt) {
        const i18n = I18nContext.current();
        throw new BadRequestException(
    ...
    const initialStatus = createMaintenanceDto.appointmentId
    ...
    const maintenance = new this.maintenanceModel({
      ...
      appointment: createMaintenanceDto.appointmentId
        ? (createMaintenanceDto.appointmentId as any)
        : null,
      sale: createMaintenanceDto.saleId
        ? (createMaintenanceDto.saleId as any)
        : null,
      createdBy: userId as any,
      ...
      statusHistory: [
        {
          ...
          changedBy: userId as any,
          notes:
            createMaintenanceDto.notes || 'Creación de orden de mantenimiento',
        },
      ],
    });

    return (await maintenance.save()).populate([
      'customer',
      'vehicle',
      'createdBy',
      'appointment',
  ...
  async findAll(
    ...
    if (filters.search && filters.search.trim()) {
      ...
      const searchConditions: any[] = [{ notes: regex }];
      if (vehicleIds.length > 0)
        searchConditions.push({ vehicle: { $in: vehicleIds } });
  ...
  async findById(id: string, branchId: string): Promise<MaintenanceDocument> {
    if (!Types.ObjectId.isValid(id)) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.invalidMaintenanceId')
          : 'ID de orden de mantenimiento inválido',
      );
    }
    ...
    if (!order) {
      ...
      throw new NotFoundException(
        i18n
          ? i18n.t('common.errors.maintenanceNotFound')
  ...
  async update(
    ...
    if (
      ...
      updateMaintenanceDto.status !== order.status
    ) {
      // Block manual status changes while the appointment hasn't been completed yet
      if (
        order.status === 'awaiting_appointment' &&
        updateMaintenanceDto.status !== 'awaiting_appointment'
      ) {
        const i18n = I18nContext.current();
        throw new BadRequestException(
          i18n
      ...
      order.statusHistory.push({
        ...
        changedBy: userId as any,
        notes:
          updateMaintenanceDto.notes ||
          `Estado actualizado a ${updateMaintenanceDto.status}`,
      });
  ...
  async linkSale(
    ...
    if (dto.saleId) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.saleIdOrFolioRequired')
          : 'Debe proporcionar saleId o folio de la venta',
      );
    }

    order.sale = sale._id.toString();
    if (!order.statusHistory) {
      order.statusHistory = [];
    }
    order.statusHistory.push({
  ...
  async notifyClient(
    ...
    order.statusHistory.push({
      ...
      notes:
        notes ||
        'Se notificó al cliente que su vehículo está listo para entrega',
    });
    order.markModified('statusHistory');
        ...
        } as any,
        {
          ...
          $push: {
            statusHistory: {
              ...
              changedAt: now,
              notes:
                'Cita completada, vehículo ingresado formalmente a cola de taller',
            },
          },
        },
        { new: true },
  ...
  async addItemUsed(
    ...
    if (order.status === 'delivered') {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.maintenanceAlreadyDelivered')
          : 'No se pueden añadir productos a una orden que ya fue entregada',
      );
    }

    // Find product to check stock & prices
    const product = await this.inventoryService.findProductById(
      addItemUsedDto.productId,
      branchId,
    );
  ...
  async addEvidencePhotos(
    ...
    const stageIndex = order.evidencePhotos.findIndex(
      (item) => item.stage.toLowerCase() === stage.toLowerCase(),
    );
  ...
  async findClientView(
    serialOrPhone: string,
    branchId: string,
  ): Promise<MaintenanceDocument[]> {
    const query: any = { branch: branchId };
    ...
    if (Types.ObjectId.isValid(input)) {
      ...
      const vehicles = await this.vehiclesService.findAll(branchId, {
        search: input,
      });
    ...
    const results = await this.maintenanceModel
      ...
      .select(
        'customer vehicle appointment status laborCost evidencePhotos notes startDate endDate',
      )
  ...
  async remove(id: string, branchId: string): Promise<void> {
    ...
    if (order.status === 'in_progress' || order.status === 'completed') {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.maintenanceCannotDelete')
          : 'No se puede eliminar una orden de servicio en curso o completada',
      );
    }
    await this.maintenanceModel

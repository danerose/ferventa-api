...
export class MaintenanceController {
  ...
  @ApiOperation({
    summary:
      'Consultar el avance y fotos de mantenimiento por últimos 4 dígitos del número de serie o celular del cliente (Público)',
  })
  trackPublicly(
    ...
    if (!q) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.searchQueryRequired')
  ...
  @ApiOperation({
    summary:
      'Recepción directa de vehículo sin cita previa (Walk-in / Mostrador)',
  })
  directReception(
    ...
    return this.maintenanceService.directReception(
      createDirectReceptionDto,
      userId,
      branchId,
    );
  }
  ...
  @ApiOperation({
    summary:
      'Listar órdenes de mantenimiento con filtros avanzados de estado, fechas y vistas',
  })
  findAll(
    @BranchId() branchId: string,
  ...
  @ApiOperation({
    summary:
      'Vincular un ticket de compra / venta POS a la orden de mantenimiento',
  })
  linkSale(
    @BranchId() branchId: string,
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary:
      'Desvincular el ticket de compra / venta POS de la orden de mantenimiento',
  })
  unlinkSale(
    @BranchId() branchId: string,
    @Param('id') id: string,
  ...
  addDiagnosticNote(
    ...
    return this.maintenanceService.addDiagnosticNote(
      id,
      branchId,
      dto.note,
      userId,
  ...
  @ApiOperation({
    summary:
      'Registrar refacción/insumo usado (Descuenta stock automáticamente)',
  })
  addItemUsed(
    @BranchId() branchId: string,
    @Param('id') id: string,
    ...
    return this.maintenanceService.addItemUsed(
      id,
      branchId,
      addItemUsedDto,
      userId,
        ...
        if (!file.mimetype.match(/\/(jpg|jpeg|png)$/)) {
          ...
          return cb(
            new BadRequestException(
              i18n
                ? i18n.t('common.errors.onlyImagesAllowed')
  ...
  @ApiOperation({
    summary:
      'Subir evidencia fotográfica por etapa de mantenimiento (Máx. 5 fotos)',
  })
  @ApiBody({
    schema: {
      type: 'object',
  ...
  uploadEvidence(
    ...
    if (!files || files.length === 0) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.photoEvidenceRequired')
          : 'Se requiere al menos una foto de evidencia',
      );
    }
    const photoUrls = files.map((file) => `/uploads/evidence/${file.filename}`);
    return this.maintenanceService.addEvidencePhotos(
      id,
      branchId,
      uploadEvidenceDto.stage,
      photoUrls,
    );
  }

...
export class AttendanceService {
  ...
  async clockIn(
    ...
    const branchObjId = isValidObjectId(branchId)
    ...
    if (activeSession) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.alreadyClockedIn')
          : 'Ya tienes un registro de entrada activo',
      );
    }
  ...
  async clockOut(
    ...
    if (!activeSession) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.notClockedIn')
          : 'No has registrado tu entrada para este turno',
      );
    }
    ...
    const totalWorkMinutes = Math.max(
      0,
      Math.round((clockOutTime - clockInTime) / 60000),
    );
    ...
    activeSession.netWorkMinutes = Math.max(
      0,
  ...
  async startBreak(
    ...
    if (activeSession.status === 'on_break') {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.alreadyOnBreak')
          : 'Ya te encuentras en un descanso',
      );
    }
  ...
  async endBreak(userId: string): Promise<AttendanceDocument> {
    ...
    if (!activeSession || activeSession.status !== 'on_break') {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.notOnBreak')
          : 'No te encuentras en un descanso actualmente',
      );
    }
    ...
    if (currentBreak) {
      ...
      currentBreak.durationMinutes = Math.max(
        0,
        Math.round((bEnd - bStart) / 60000),
      );
    }
  ...
  async getTodayStatus(userId: string) {
    ...
    if (activeSession) {
      const now = new Date();
      const clockInTime = new Date(activeSession.clockIn).getTime();
      const currentWorkMinutes = Math.max(
        0,
        Math.round((now.getTime() - clockInTime) / 60000),
      );
      ...
      if (ongoingBreak) {
        ...
        const ongoingBreakMinutes = Math.max(
          0,
          Math.round((now.getTime() - bStart) / 60000),
        );
        activeBreak = {
      ...
      return {
        ...
        netWorkHours: Number(
          (Math.max(0, currentWorkMinutes - totalBreakMinutes) / 60).toFixed(2),
        ),
        activeBreak,
      };
    }
  ...
  async getBranchTodayStatus(branchId: string) {
    if (!branchId) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.branchIdRequired')
          : 'Se requiere el ID de la sucursal',
      );
    }
    ...
    for (const [uId, userObj] of allUsersMap.entries()) {
      ...
      if (record && !record.clockOut) {
        ...
        userStatuses.push({
          ...
          netWorkMinutes: Math.max(0, currentWorkMinutes - totalBreakMinutes),
          netWorkHours: Number(
            (Math.max(0, currentWorkMinutes - totalBreakMinutes) / 60).toFixed(
              2,
            ),
          ),
          activeBreak,
        ...
        userStatuses.push({
          ...
          totalWorkHours: Number(
            ((record.totalWorkMinutes || 0) / 60).toFixed(2),
          ),
          totalBreakMinutes: record.totalBreakMinutes || 0,
          totalBreakHours: Number(
            ((record.totalBreakMinutes || 0) / 60).toFixed(2),
          ),
  ...
  async getAdminRecords(
    ...
    if (queryDto.branchId) {
      query.branch = isValidObjectId(queryDto.branchId)
        ? new Types.ObjectId(queryDto.branchId)
        : queryDto.branchId;
    }
  ...
  async updateAdminRecord(
    ...
    if (!record) {
      ...
      throw new NotFoundException(
        i18n
          ? i18n.t('common.errors.recordNotFound')
    ...
    if (record.clockIn && record.clockOut) {
      ...
      record.netWorkMinutes = Math.max(
        0,
        totalWorkMinutes - (record.totalBreakMinutes || 0),
      );
    }

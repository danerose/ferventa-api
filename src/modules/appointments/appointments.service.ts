...
} from './schemas/workshop-schedule.schema';
import {
  WorkshopHoliday,
  WorkshopHolidayDocument,
} from './schemas/workshop-holiday.schema';
...
export class AppointmentsService {
  constructor(
    ...
    @InjectModel(WorkshopSchedule.name)
    private scheduleModel: Model<WorkshopScheduleDocument>,
    @InjectModel(WorkshopHoliday.name)
    private holidayModel: Model<WorkshopHolidayDocument>,
  ...
  async validateBookingTime(
    ...
    // 1. Check if holiday
    const isHoliday = await this.holidayModel
      .findOne({ date: dateStr, branch: branchId })
      .exec();
    if (isHoliday) {
      const i18n = I18nContext.current();
      const message = i18n
        ? i18n.t('common.errors.holidayClosure', {
            args: { date: dateStr, description: isHoliday.description },
          })
    ...
    const schedule = await this.scheduleModel
      .findOne({ dayOfWeek, branch: branchId })
      .exec();
    if (!schedule || !schedule.isWorking) {
      const i18n = I18nContext.current();
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.nonWorkingDay')
          : 'El taller no labora en este día de la semana',
      );
    }
    ...
    if (
      ...
      const i18n = I18nContext.current();
      const message = i18n
        ? i18n.t('common.errors.outsideWorkingHours', {
            args: { start: schedule.startTime, end: schedule.endTime },
          })
    ...
    const sameDayAppointments = await this.appointmentModel
      .find({
        branch: branchId,
    ...
    for (const appt of sameDayAppointments) {
      ...
      const apptEnd = new Date(
        apptStart.getTime() + (appt.duration || 15) * 60 * 1000,
      );
      ...
      if (apptStart < appointmentEnd && apptEnd > appointmentStart) {
        ...
        const message = i18n
          ? i18n.t('common.errors.appointmentOverlap', {
              args: { start: startStr, end: endStr },
            })
  ...
  async create(
    ...
    try {
      ...
      if (initialStatus === 'completed') {
        await this.maintenanceService.activateFromAppointment(
          (savedAppt._id as any).toString(),
          branchId,
        );
      }
      ...
      console.error(
        'Error creating maintenance automatically from appointment:',
        err,
      );
  ...
  async findById(id: string, branchId: string): Promise<AppointmentDocument> {
    if (!Types.ObjectId.isValid(id)) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.invalidAppointmentId')
          : 'ID de cita inválido',
      );
    }
    ...
    if (!appointment) {
      ...
      throw new NotFoundException(
        i18n
          ? i18n.t('common.errors.appointmentNotFound')
          : 'Cita no encontrada',
      );
    }
    return appointment;
  ...
  async update(
    ...
    if (updateAppointmentDto.customerPhone)
      appointment.customerPhone = updateAppointmentDto.customerPhone;
    if (updateAppointmentDto.customerEmail)
      appointment.customerEmail = updateAppointmentDto.customerEmail;
    if (updateAppointmentDto.whatsappId !== undefined)
      appointment.whatsappId = updateAppointmentDto.whatsappId;
    if (updateAppointmentDto.serviceRequested)
      appointment.serviceRequested = updateAppointmentDto.serviceRequested;
    if (updateAppointmentDto.status)
      appointment.status = updateAppointmentDto.status;
    if (updateAppointmentDto.notes !== undefined)
      appointment.notes = updateAppointmentDto.notes;
    if (updateAppointmentDto.branchName !== undefined)
      appointment.branchName = updateAppointmentDto.branchName;
  ...
  async addHoliday(
    ...
    description: string,
    branchId: string,
  ): Promise<WorkshopHoliday> {
    const existing = await this.holidayModel
      .findOne({ date, branch: branchId })
      .exec();
    if (existing) {
      const i18n = I18nContext.current();
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.holidayAlreadyRegistered')
          : 'Esta fecha ya está registrada como día festivo',
      );
    }
  ...
  async removeHoliday(id: string, branchId: string): Promise<void> {
    const i18n = I18nContext.current();
    if (Types.ObjectId.isValid(id)) {
      const res = await this.holidayModel
        .findOneAndDelete({ _id: id, branch: branchId })
        .exec();
      if (!res)
        throw new NotFoundException(
          i18n
            ? i18n.t('common.errors.holidayNotFound')
            : 'Día festivo no encontrado',
        );
    } else {
      const res = await this.holidayModel
        .findOneAndDelete({ date: id, branch: branchId })
        .exec();
      if (!res)
        throw new NotFoundException(
          i18n
  ...
  async getOccupiedSlots(
    ...
    const weeklySchedules = await this.scheduleModel
      .find({ branch: branchId })
      .exec();
  ...
  async checkIn(
    ...
    let maintenance = await this.maintenanceService.activateFromAppointment(
      id,
      branchId,
      receptionNotes,
    );
    ...
    if (!maintenance) {
      ...
      if (vehicle && customerId) {
        ...
        maintenance = await this.maintenanceService.activateFromAppointment(
          (saved._id as any).toString(),
          branchId,
          receptionNotes,
        );

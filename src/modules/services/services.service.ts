...
export class ServicesService {
  constructor(
    @InjectModel(PredefinedService.name)
    private serviceModel: Model<PredefinedServiceDocument>,
  ...
  async create(
    ...
    branchId: string,
  ): Promise<PredefinedServiceDocument> {
    const supplies = await this.validateSupplies(
      createServiceDto.supplies,
      branchId,
    );
  ...
  async findAll(filters: {
    ...
    page?: number;
    limit?: number;
  }): Promise<PaginatedResult<PredefinedServiceDocument>> {
    ...
    if (filters.search) {
      query.$or = [{ name: buildFuzzyRegex(filters.search) }];
    }
  ...
  async findById(id: string): Promise<PredefinedServiceDocument> {
    if (!Types.ObjectId.isValid(id)) {
      ...
      throw new BadRequestException(
        i18n ? i18n.t('common.errors.invalidId') : 'ID inválido',
      );
    }
    ...
    if (!service) {
      ...
      throw new NotFoundException(
        i18n
          ? i18n.t('common.errors.serviceNotFound')
  ...
  async update(
    ...
    if (updateServiceDto.supplies) {
      service.supplies = await this.validateSupplies(
        updateServiceDto.supplies,
        branchId,
      );
    }

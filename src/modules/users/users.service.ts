...
export class UsersService implements OnModuleInit {
  ...
  private async seedAdminUser() {
    ...
    if (adminCount === 0) {
      ...
      if (adminRole) {
        ...
        console.log(
          'Default admin user seeded: admin@ferventa.com / AdminPassword123!',
        );
      }
    }
  }
  ...
  async findByUsername(username: string): Promise<UserDocument | null> {
    ...
    return this.userModel
      .findOne({ username: username.toLowerCase().trim(), deletedAt: null })
      .populate('role')
      .exec();
  }
  ...
  async generateUsername(name: string): Promise<string> {
    ...
    if (parts.length >= 2) {
      base = `${parts[0]}.${parts[parts.length - 1]}`.replace(
        /[^a-z0-9.]/g,
        '',
      );
    } else {
  ...
  async checkUsername(
    ...
    if (!username || !username.trim()) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.badRequest')
  ...
  async create(createUserDto: CreateUserDto): Promise<{
    ...
    if (username) {
      ...
      if (existingUsername) {
        const i18n = I18nContext.current();
        const message = i18n
          ? i18n.t('common.errors.usernameRegistered')
    ...
    if (existingEmail) {
      const i18n = I18nContext.current();
      const message = i18n
        ? i18n.t('common.errors.emailRegistered')
    ...
    if (!role) {
      ...
      throw new NotFoundException(
        i18n
          ? i18n.t('common.errors.roleNotFound')
    ...
    try {
      ...
      if (err?.code === 11000) {
        ...
        const isUsername =
          ...
          JSON.stringify(err.keyValue).includes('username');
...
🔗 https://app.ferventa.com/login`;
  ...
  async migrateUsernames(): Promise<{
    ...
    for (const user of usersWithoutUsername) {
      ...
      console.log(
        `Migrated user '${user.name}' -> username: '${generatedUsername}'`,
      );
    }

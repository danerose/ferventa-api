...
export class CreateUserDto {
  ...
  @ApiProperty({
    example: 'arojas',
    description:
      'Nombre de usuario único (opcional, se auto-genera si no se provee)',
    required: false,
  })
  @IsString({ message: i18nValidationMessage('validation.isString') })
  @IsOptional()
  ...
  @ApiProperty({
    ...
    description:
      'ID o nombre del Rol asignado (ej. mechanic, admin, seller, warehouse)',
  })
  @IsString({ message: i18nValidationMessage('validation.isString') })
  @IsNotEmpty({ message: i18nValidationMessage('validation.isNotEmpty') })
  roleId: string;
  ...
  @ApiProperty({
    ...
    description: 'IDs de las sucursales a las que tiene acceso el usuario',
    type: [String],
  })
  @IsArray()
  @IsMongoId({

...
export class OrdersService {
  ...
  async create(
    ...
    if (advancePayment < minAdvanceRequired) {
      const currentPercent =
        Math.round((advancePayment / sellingPrice) * 1000) / 10;
    ...
    const remainingBalance =
      Math.round((sellingPrice - advancePayment) * 100) / 100;
    ...
    const order = new this.orderModel({
      ...
      estimatedArrivalDate: createOrderDto.estimatedArrivalDate
        ? new Date(createOrderDto.estimatedArrivalDate)
        : null,
  ...
  async addPayment(
    ...
    if (order.status === OrderStatus.CANCELLED) {
      throw new BadRequestException(
        'No se pueden registrar pagos a un pedido cancelado',
      );
    }

    if (order.isFullyPaid) {
      throw new BadRequestException(
        'Este pedido ya se encuentra completamente liquidado (100%)',
      );
    }
    ...
    const newAdvance =
      Math.round((order.advancePayment + paymentAmount) * 100) / 100;
    const newRemaining =
      Math.round((order.sellingPrice - newAdvance) * 100) / 100;
    const newPercentage =
      Math.round((newAdvance / order.sellingPrice) * 1000) / 10;
  ...
  async cancel(
    ...
    if (order.status === OrderStatus.DELIVERED) {
      throw new BadRequestException(
        'No se puede cancelar un pedido que ya fue entregado al cliente',
      );
    }
  ...
  async update(
    ...
    if (dto.estimatedArrivalDate !== undefined) {
      order.estimatedArrivalDate = dto.estimatedArrivalDate
        ? new Date(dto.estimatedArrivalDate)
        : null;
    }
    ...
    if (dto.sellingPrice !== undefined || dto.costPrice !== undefined) {
      ...
      if (dto.sellingPrice !== undefined) {
        ...
        order.minAdvanceRequired =
          Math.round(newSellingPrice * 0.5 * 100) / 100;
        order.remainingBalance =
          Math.round((newSellingPrice - order.advancePayment) * 100) / 100;
        order.advancePercentage =
          Math.round((order.advancePayment / newSellingPrice) * 1000) / 10;

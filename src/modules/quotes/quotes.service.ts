...
export class QuotesService {
  ...
  async create(
    ...
    for (const item of createQuoteDto.items) {
      ...
      if (!product.isActive) {
        ...
        const message = i18n
          ? i18n.t('common.errors.productInactive', {
              args: { sku: product.sku },
            })
    ...
    if (total < 0) {
      ...
      throw new BadRequestException(
        i18n
          ...
          : 'El descuento total no puede superar el subtotal de la cotización',
      );
    }
  ...
  async findById(id: string, branchId?: string): Promise<QuoteDocument> {
    if (!Types.ObjectId.isValid(id)) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.invalidQuoteId')
    ...
    if (!quote) {
      ...
      throw new NotFoundException(
        i18n
          ? i18n.t('common.errors.quoteNotFound')
  ...
  async update(
    ...
    if (quote.status === 'converted_to_sale') {
      ...
      throw new BadRequestException(
        i18n
          ...
          : 'No se puede modificar una cotización que ya ha sido convertida en venta',
      );
    }
    ...
    if (updateQuoteDto.items) {
      ...
      const globalDiscount =
        updateQuoteDto.globalDiscount !== undefined
          ? updateQuoteDto.globalDiscount
          : quote.discount - itemsDiscount;

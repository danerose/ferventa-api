...
export class ReportsService {
  ...
  async getSalesSummary(start: string, end: string) {
    ...
    if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
      ...
      throw new BadRequestException(
        i18n
          ? i18n.t('common.errors.invalidDateFormat')
    ...
    return {
      ...
      dailySales: dailyStats.map((item) => ({
        date: item._id,
  ...
  async getMaintenanceSummary(branchId?: string) {
      ...
      },
      {
        $match: {
          $or: [
            ...
            {
              'apptDoc.0.status': {
                $in: ['approved', 'completed', 'rescheduled'],
              },
            },
          ],
        },
    ...
    return stats.reduce((acc, curr) => {
      acc[curr._id] = {
        count: curr.count,
        avgLaborCost: Math.round(curr.avgLaborCost),
      };
      return acc;
    }, {});
  }
  ...
  async getMaintenanceMetrics(branchId?: string, start?: string, end?: string) {
    ...
    orders.forEach((o) => {
      ...
      const delivDate =
        o.deliveredAt || (o.status === 'delivered' ? o.endDate : null);

      if (recDate && started) {
        const diffMs =
          new Date(started).getTime() - new Date(recDate).getTime();
        if (diffMs >= 0) {
      ...
      if (started && compDate) {
        const diffMs =
          new Date(compDate).getTime() - new Date(started).getTime();
        if (diffMs >= 0) {
      ...
      if (compDate && delivDate) {
        const diffMs =
          new Date(delivDate).getTime() - new Date(compDate).getTime();
        if (diffMs >= 0) {
      ...
      if (recDate && delivDate) {
        const diffMs =
          new Date(delivDate).getTime() - new Date(recDate).getTime();
        if (diffMs >= 0) {
    ...
    const pendingPickupVehicles = pendingPickupOrders.map((order) => {
      ...
      const daysWaiting = Math.max(
        0,
        Math.floor((now - finishMs) / (1000 * 60 * 60 * 24)),
      );

      const notifiedDate = order.notifiedAt
        ? new Date(order.notifiedAt).getTime()
        : null;
      const daysSinceNotified = notifiedDate
        ? Math.max(0, Math.floor((now - notifiedDate) / (1000 * 60 * 60 * 24)))
        : null;
      ...
      return {
        ...
        vehicle:
          `${(order.vehicle as any)?.brand || ''} ${(order.vehicle as any)?.model || ''} (${(order.vehicle as any)?.serialNumberLastFour || ''})`.trim(),
    ...
    return {
      ...
      averages: {
        avgQueueHours:
          queueCount > 0 ? +(totalQueueHours / queueCount).toFixed(1) : 0,
        avgQueueDays:
          queueCount > 0 ? +(totalQueueHours / queueCount / 24).toFixed(1) : 0,
        avgWorkHours:
          workCount > 0 ? +(totalWorkHours / workCount).toFixed(1) : 0,
        avgWorkDays:
          workCount > 0 ? +(totalWorkHours / workCount / 24).toFixed(1) : 0,
        avgPickupHours:
          pickupCount > 0 ? +(totalPickupHours / pickupCount).toFixed(1) : 0,
        avgPickupDays:
          pickupCount > 0
            ? +(totalPickupHours / pickupCount / 24).toFixed(1)
            : 0,
        avgTotalStayHours:
          stayCount > 0 ? +(totalStayHours / stayCount).toFixed(1) : 0,
        avgTotalStayDays:
          stayCount > 0 ? +(totalStayHours / stayCount / 24).toFixed(1) : 0,
      },

...
export class OrdersController {
  ...
  @Post()
  @Roles('admin', 'seller')
  @ApiOperation({ summary: 'Levantar un nuevo pedido especial con anticipo mínimo del 50%' })
  @ApiResponse({ status: 201, description: 'Pedido creado exitosamente con anticipo registrado.' })
  create(
  ...
  @ApiOperation({ summary: 'Obtener métricas y resumen de pedidos por estatus y saldos pendientes' })
  ...
  @ApiQuery({ name: 'search', required: false, description: 'Buscar por folio, cliente, teléfono o descripción de pieza' })
  @ApiQuery({ name: 'status', required: false, description: 'Filtrar por estado' })
  @ApiQuery({ name: 'isFullyPaid', required: false, type: Boolean, description: 'Filtrar si está liquidado (true/false)' })
  ...
  findAll(
    ...
    const isPaidBool = isFullyPaid !== undefined ? isFullyPaid === 'true' : undefined;
  ...
  @Patch(':id/status')
  @Roles('admin', 'seller', 'warehouse')
  @ApiOperation({ summary: 'Actualizar el estado del pedido (Pedido Levantado -> Pedido -> En tránsito -> En Sucursal -> Pendiente de entrega -> Entregado)' })
  updateStatus(
    @BranchId() branchId: string,
    @Param('id') id: string,
  ...
  @Post(':id/payments')
  @Roles('admin', 'seller')
  @ApiOperation({ summary: 'Registrar un abono o liquidación al saldo del pedido' })
  addPayment(
    @BranchId() branchId: string,
    @Param('id') id: string,

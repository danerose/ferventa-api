...
export class AttendanceController {
  ...
  @ApiOperation({ summary: 'Registrar entrada (Clock In) para el turno del usuario' })
  ...
  async clockIn(
    ...
    const data = await this.attendanceService.clockIn(targetUserId, branchId, dto);
    const i18n = I18nContext.current();
    return {
      message: i18n ? i18n.t('common.success.attendance.clockIn') : 'Entrada registrada exitosamente',
      data,
    };
  }

  @Post('clock-out')
  @ApiOperation({ summary: 'Registrar salida (Clock Out) del turno del usuario' })
  ...
  async clockOut(
    ...
    const i18n = I18nContext.current();
    return {
      message: i18n ? i18n.t('common.success.attendance.clockOut') : 'Salida registrada exitosamente',
      data,
    };
  }

  @Post('break/start')
  @ApiOperation({ summary: 'Iniciar descanso / hora de comida durante el turno' })
  ...
  async startBreak(
    ...
    const i18n = I18nContext.current();
    return {
      message: i18n ? i18n.t('common.success.attendance.startBreak') : 'Inicio de descanso registrado exitosamente',
      data,
    };
  }
  ...
  async endBreak(
    ...
    const i18n = I18nContext.current();
    return {
      message: i18n ? i18n.t('common.success.attendance.endBreak') : 'Fin de descanso registrado exitosamente',
      data,
    };
  }
  ...
  async getMyRecords(
    ...
    const data = await this.attendanceService.getMyRecords(userId, startDate, endDate);
    return { data };
  }

  @Get('admin/records')
  @Roles('admin')
  @ApiOperation({ summary: 'Listar todos los registros de asistencia con filtros (Solo Admin)' })
  ...
  @Get('admin/summary')
  @Roles('admin')
  @ApiOperation({ summary: 'Obtener resumen de horas trabajadas (semanal, quincenal, mensual) (Solo Admin)' })
  ...
  @Roles('admin')
  @ApiOperation({ summary: 'Obtener desglose detallado de asistencia y descansos para un usuario (Solo Admin)' })
  @ApiQuery({ name: 'startDate', required: false, example: '2026-07-01' })
  @ApiQuery({ name: 'endDate', required: false, example: '2026-07-31' })
  async getAdminUserBreakdown(
    ...
    const data = await this.attendanceService.getAdminUserBreakdown(userId, startDate, endDate);
    return { data };
  }

  @Patch('admin/:id')
  @Roles('admin')
  @ApiOperation({ summary: 'Modificar/Ajustar manualmente un registro de asistencia (Solo Admin)' })
  async updateAdminRecord(
    ...
    const i18n = I18nContext.current();
    return {
      message: i18n ? i18n.t('common.success.attendance.updated') : 'Registro de asistencia actualizado exitosamente',
      data,
    };
  }
}

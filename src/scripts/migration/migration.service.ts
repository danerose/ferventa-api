...
import { Role, RoleDocument } from '../../modules/users/schemas/role.schema';
...
export class MigrationService {
  ...
  constructor(
    ...
    private workshopHolidayModel: Model<WorkshopHolidayDocument>,
    @InjectModel(WorkshopSchedule.name)
    private workshopScheduleModel: Model<WorkshopScheduleDocument>,
  ...
  async migrateToBranches() {
    ...
    let defaultBranch = await this.branchModel.findOne({
    ...
    if (adminRole) {
      ...
      for (const admin of admins) {
        ...
        if (!branches.some((id) => id.toString() === branchId.toString())) {
          ...
          this.logger.log(
            `Assigned default branch to admin user: ${admin.email}`,
          );
        }
      }
    }
    ...
    for (const collection of collections) {
      ...
      this.logger.log(
        `Migrated ${result.modifiedCount} records in ${collection.name}`,
      );
    }

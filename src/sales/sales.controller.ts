...
export class SalesController {
  ...
  cancel(
    ...
    return this.salesService.cancel(id, cancelSaleDto, userId);
  }
  ...
  findAll(
    ...
    return this.salesService.findAll({ customerId, isCancelled: isCancelledBool });
  }
  ...
  async getTicket(@Param('query') query: string) {
    if (!query) {
    ...
    try {
      return await this.salesService.findByFolio(query);
    } catch (e) {
      return this.salesService.findById(query);
    }
  }
}

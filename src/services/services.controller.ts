...
export class ServicesController {
  ...
  @Get()
  @ApiOperation({ summary: 'Listar todos los servicios predefinidos paginados' })
  @ApiQuery({ name: 'isActive', required: false, type: Boolean })
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Búsqueda por nombre de servicio' })
  @ApiQuery({ name: 'q', required: false, type: String, description: 'Alias para término de búsqueda' })
  ...
  async searchByName(
    ...
    const result = await this.servicesService.searchByName(q || '', activeFilter, query?.page, query?.limit);
    const i18n = I18nContext.current();
    return {
      success: true,
      data: result,
